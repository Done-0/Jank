...
- 多个文件的包中，包注释只需在一个文件中出现（通常是与包同名的文件）。
- 包注释必须包含以下信息（按顺序）：
    - 包的基本简介（包名及功能说明）
    - 创建者信息，格式：创建人：[姓名]
...
```go
// utils 包，该包包含了项目共用的一些常量，封装了项目中一些共用函数。
// 创建人：Fender
// 创建时间：20250325
```
...
- 每个函数或方法必须有注释说明，包含以下内容（按顺序）：
    - 简要说明：以函数名开头，用逗号分隔说明部分
    - 参数列表：每个参数单独一行，参数名开头，用逗号分隔说明部分
    - 返回值：每个返回值单独一行
